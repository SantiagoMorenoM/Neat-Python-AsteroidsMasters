...
class Asteroids():
    ...
    def playGame(self, genomes, config):
        ...
        best = pd.read_pickle(r'best.pickle')        
        for _, g in genomes:
            net = neat.nn.FeedForwardNetwork.create(g, config)
            nets.append(net)
            currShip=Ship(self.stage)
            
            ships.append(currShip)
        ...
        for x, ship in enumerate(ships):
            ...
            while not lastLive:
                ...
                for rock in self.rockList:
                    radius, angle = rock.find_polar(self.ship)
                    if i[0] > radius:
                        i[0] = radius
                        i[1] = angle
                if self.saucer is not None:
                    radius, angle = self.saucer.find_polar(self.ship)
                    if i[0] > radius:
                        i[0] = radius
                        i[1] = angle
                output = nets[x].activate(i)
                if self.bestOn:
                    output = best.activate(i)
                if self.randomized:
                ...
                for rock in self.rockList:
                    ...
                    if (tick >= 1000 or tick2 >= 1000 ) and self.lives>0 and (not self.bestOn and not self.randomized):
                        ge[x].fitness -= 5000
                        end= True
                        self.killShip()
                        
                   
                if self.lives==0 or end:
                    ge[x].fitness -= 10000-self.score
                    #if self.score<=50:
                       # ge[x].fitness -= 50000
                    ships.pop(x)
                    nets.pop(x)
                ...
                pygame.display.flip()
                if self.score > 12000 and (not self.bestOn and not self.randomized):  # break program and save neural network if it becomes good enough
    ...
    def displayScore(self):
        ...
        numStr = str("num: " + str(self.number))
        genStr = str("gen: " + str(GEN))
        scoreText = font1.render(scoreStr, True, (200, 200, 200))
        numText = font1.render(numStr, True, (200, 200, 200))
        genText = font1.render(genStr, True, (200, 200, 200))
        scoreTextRect = scoreText.get_rect(centerx=100, centery=45)
        numTextRect = scoreText.get_rect(centerx=300, centery=45)
        genTextRect = scoreText.get_rect(centerx=500, centery=45)
        self.stage.screen.blit(scoreText, scoreTextRect)
        self.stage.screen.blit(numText, numTextRect)
        self.stage.screen.blit(genText, genTextRect)
...
def run(Asteroids, config_path):
    ...
    pygame.display.toggle_fullscreen()
    winner = population.run(Asteroids.playGame, 50)
